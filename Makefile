...
# The name of the program you're buiing, usually the file which contains main(). 
...
SRC = $(TARGET).c task_comms.c task_sensors.c task_motors.c task_orient.c task_safety.c task_master.c pid.c uart.c twi.c
#task_user.cpp task_master.cpp 
...
$(TARGET).elf:  library $(OBJS)
	avr-gcc $(OBJS) $(LIB_NAME) -g -lm -mmcu=$(MCU) -o $(TARGET).elf 
...
install:  $(TARGET).hex
  ...
  else ifeq ($(PROG), buspirate)
	avrdude -p $(MCU) -P /dev/ttyUSB0 -c buspirate -V -B 1 -Uflash:w:$(TARGET).hex
  else
